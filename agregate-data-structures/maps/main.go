...
func main() {
	...
	//NOTE: maps are unordered - they always appear random when adding new key
	// order is not gauranteed

	//creating a map
	fmt.Println("Using maps")
	// 1st posibility
	mymap := make(map[string][]string) // this will create map with str key and str slice values

	// 2nd posibility using make
	...
	// optional capacity
	// mymap := make(map[string]bool, 100)

	mymap[`bond_james`] = []string{`shakedn, not stirred`, `martinis`, `fast cars`}
	mymap[`monnypenny_miss`] = []string{`intelligence`, `literature`, `computer science`}
	mymap[`no_dr`] = []string{`cats`, `ice cream`, `sunsets`}

	fmt.Printf("Whole map:\n %#v\n\n", mymap)
	...
	fmt.Println("Add records to map")

	// add by creating new key
